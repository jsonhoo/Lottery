...
public class ConfigModel {
    ...
    public static void discoverDevice(final int deviceId) {
        ...
        App.bus.post(new MeshRequestEvent(MeshRequestEvent.RequestEvent.CONFIG_DISCOVER_DEVICE, data));
    }
    ...
    public static int getInfo(final int deviceId, DeviceInfo infoType) {
        ...
        App.bus.post(new MeshRequestEvent(MeshRequestEvent.RequestEvent.CONFIG_GET_INFO, data));
        return id;
    }

    public static void resetDevice(String deviceId, long deviceHash, byte[] dmKey) {
        Bundle data = new Bundle();
        data.putString(MeshConstants.EXTRA_DEVICE_ID, deviceId);
        data.putLong(MeshConstants.EXTRA_UUIDHASH_64, deviceHash);
        data.putByteArray(MeshConstants.EXTRA_RESET_KEY, dmKey);
        App.bus.post(new MeshRequestEvent(MeshRequestEvent.RequestEvent.CONFIG_RESET_DEVICE, data));
    }
    ...
    public static void setDeviceId(int deviceId, long uuidHash, final int newDeviceId) {
        ...
        App.bus.post(new MeshRequestEvent(MeshRequestEvent.RequestEvent.CONFIG_SET_DEVICE_IDENTIFIER, data));
    }
    ...
    public static int getParameters(final int deviceId) {
        ...
        data.putInt(MeshLibraryManager.EXTRA_REQUEST_ID, id);
        data.putInt(MeshConstants.EXTRA_DEVICE_ID, deviceId);
        App.bus.post(new MeshRequestEvent(MeshRequestEvent.RequestEvent.CONFIG_GET_PARAMETERS, data));
        return id;
    }
    ...
    public static int setParameters(final int deviceId, int txInterval, int txDuration, int rxDutyCycle, int txPower, int ttl) {
        ...
        App.bus.post(new MeshRequestEvent(MeshRequestEvent.RequestEvent.CONFIG_SET_PARAMETERS, data));
        return id;
    }
